...
-- 1.SELECT 문장 작성법
SELECT * FROM TAB;      -- 현계정(SCOTT)이 가지고 있는 테이블 정보(실행 : CTRL+ENTER)
...
SELECT EMPNO, ENAME, JOB 
  FROM EMP; -- EMP 테이블의 SELECT 절에 지정된 열만 출력
SELECT EMPNO AS "사 번", ENNAME AS "이 름", SAL AS "급여", JOB AS "직책"
  FROM EMP;  -- 열이름에 별칭을 두는 경우 EX. 열이름 AS "별칭"
SELECT EMPNO "사 번", ENNAME "이 름", SAL "급여", JOB
  FROM EMP;  -- 열이름에 별칭을 두는 경우 EX. 열이름 AS "별칭", 열이름
SELECT EMPNO "사 번", ENAME 이름, SAL 급여, JOB
  FROM EMP;  -- 별칭에 SPACERK 없는 경우 따옴표 생략가능
  
-- 3.특정 행만 출력(연산포함) :WHERE절(조건절) -- 비교연산자: 같다(=),다르다(!=,^=,<>), >,..
SELECT EMPNO 사번, ENAME 이름, SAL 급여, JOB FROM EMP WHERE SAL=3000;
...
-- 부서번호(DEPTNO)가 10번인 사원의 모든 필드
SELECT * FROM EMP WHERE DEPTNO=10;
-- SQL문은 대소문자 구별없음. 데이타는 대소문자 구별
-- EX4. 이름(ENAME)이 SCOTT인 직원의 모든 데이터(필드)
SElect * FROM emP WHERE eNAME = 'SCOTT';

-- 4.WHERE절 조건절에 논리연산자 : AND OR NOT
-- EX1. 급여(SAL)가 2000이상 3000이하인 직원의 모든 필드
SELECT * FROM EMP WHERE SAL>2000 AND SAL<=3000;
-- EX2. 82년도 입사한 사원의 모든 필드
SELECT * FROM EMP WHERE HIREDATE>='82/01/10' AND HIREDATE<='82/12/31';
...
-- EX4. 부서번호가 10이 아닌 직원의 모든 필드
SELECT * FROM EMP WHERE DEPTNO != 10;
SELECT * FROM EMP WHERE NOT DEPTNO = 10;

-- 5. 산술연산자(SELECT, WHERE, ORDER BY)
...
SELECT ENAME || '은(는)' || 'XXX 업무이고 연봉은 XX다(연봉은 ' || SAL*12+NVL(COMM, 0) || ')' FROM 
   EMP;
